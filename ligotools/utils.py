...
def plot_match_results(
    ...
    """
    Plot SNR(t), whitened data + template, residuals, and ASD+template,
    saving all figures into the figures/ directory.
    This is the plotting code that originally lived in the
    '# -- To calculate the PSD of the data...' cell.
    """

    # --- SNR vs time (full, then zoom-in)
    ...
    plt.legend(loc="upper left")
    plt.title(
        det + " Residual whitened data after subtracting template around event"
    )
    plt.savefig(
        os.path.join("figures", eventname + "_" + det + "_matchtime." + plottype)
    )

    # --- ASD + template in frequency
    ...
    plt.savefig(
        os.path.join("figures", eventname + "_" + det + "_matchfreq." + plottype)
    )